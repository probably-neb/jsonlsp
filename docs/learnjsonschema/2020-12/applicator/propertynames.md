...
The [`propertyNames`](propertyNames.md) keyword restricts
...
> [`properties`](properties.md) keyword) is either
> meaningless or leads to unsatisfiable schemas. Conversely, explicitly setting
> the [`type`](../validation/type.md) keyword to `string` is redundant.
