...
The [`$ref`](ref.md) keyword enables a schema to reference another schema by its URI,
...
> and referencing purposes is through the [`$id`](id.md)
> keyword.
...
> be a subschema.  For example, referencing the contents of the [`examples`](../meta-data/examples.md) keyword.
...
[`$id`](id.md) keyword, or the base URI as determined
...
version of such URI through keywords such as [`$id`](id.md) and [`$anchor`](anchor.md). If so, the reference
