...
[`$schema`](schema.md) keyword is not declared, the schema inherits
its context-specific or implementation-specific default dialect.

> **Digging Deeper:**
>  It is common to avoid the [`$schema`](schema.md)
...
> [`$schema`](schema.md) keyword. This ensures that less strict
> implementations unambiguously know how to process the schema and don't attempt
> to guess.

Note that the [`$schema`](schema.md) keyword can occur multiple
...
> circular problem.
>
> A common occurrence of this issue is setting the [`$schema`](schema.md)
...
To debug the role of the [`$schema`](schema.md) keyword on a schema
