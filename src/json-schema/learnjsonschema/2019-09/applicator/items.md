...
- **Schema**: When set to a schema, [`items`](items.md) validates that all items in the array
...
- **Array**: When set to an array of schemas, [`items`](items.md) validates each item in the array instance
  ...
  validated using the [`additionalItems`](additionalitems.md) keyword. The annotation reports the
...
> empty. If needed, use the [`minItems`](../validation/minitems.md) to assert on the minimum bounds of the array.
