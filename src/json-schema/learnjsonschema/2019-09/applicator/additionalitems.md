...
> is set to a schema (not an array of schemas), [`additionalItems`](additionalitems.md) has no effect and is ignored.

> **Common Pitfall:**
> This keyword does not prevent an array instance from being
> empty or having fewer items than the [`items`](items.md) array. If needed, use the [`minItems`](../validation/minitems.md) keyword to assert on the minimum bounds of
> the array.
